...
CGFloat const FCXDotLineAnimationContentHeight = 40.0;
CGFloat const FCXDotLineAnimationRadius = 5.0;
CGFloat const FCXDoLinttAnimationTranslateDistance = 5.0;

@implementation FCXDotLinePathAnimation
{
    CALayer *_bgLayer;
...
- (void)addContentLayer {
    ...
    _bgLayer.frame = CGRectMake((self.frame.size.width - FCXDotLineAnimationContentHeight)/2.0, (self.frame.size.height - FCXDotLineAnimationContentHeight)/2.0, FCXDotLineAnimationContentHeight, FCXDotLineAnimationContentHeight);
    ...
    CGFloat dotSize = FCXDotLineAnimationRadius * 2;
    
    _topLayer = [self addDotLayerWithFrame:CGRectMake(_bgLayer.frame.size.width/2.0 - FCXDotLineAnimationRadius, 0, dotSize, dotSize) color:topPointColor.CGColor];
    _topLayer.hidden = NO;
    _leftLayer = [self addDotLayerWithFrame:CGRectMake(0, _bgLayer.frame.size.height/2.0 - FCXDotLineAnimationRadius, dotSize, dotSize) color:leftPointColor.CGColor];
    _bottomLayer = [self addDotLayerWithFrame:CGRectMake(_bgLayer.frame.size.width/2.0 - FCXDotLineAnimationRadius, _bgLayer.frame.size.height - dotSize, dotSize, dotSize) color:bottomPointColor.CGColor];
    _rightLayer = [self addDotLayerWithFrame:CGRectMake(_bgLayer.frame.size.width - dotSize,  _bgLayer.frame.size.height/2.0 - FCXDotLineAnimationRadius, dotSize, dotSize) color:rightPointColor.CGColor];
    
    _lineLayer = [CAShapeLayer layer];
    _lineLayer.frame = self.bounds;
    _lineLayer.lineWidth = FCXDotLineAnimationRadius * 2;
    _lineLayer.lineCap = kCALineCapRound;
    _lineLayer.lineJoin = kCALineJoinRound;
    _lineLayer.fillColor = [UIColor clearColor].CGColor;
    _lineLayer.strokeColor = topPointColor.CGColor;
    UIBezierPath * path = [UIBezierPath bezierPath];
    CGPoint topPoint = CGPointMake(_bgLayer.frame.size.width/2.0, FCXDotLineAnimationRadius);
    [path moveToPoint:topPoint];
    [path addLineToPoint:CGPointMake(FCXDotLineAnimationRadius, _bgLayer.frame.size.height/2.0)];
    [path addLineToPoint:CGPointMake(_bgLayer.frame.size.width/2.0, _bgLayer.frame.size.height - FCXDotLineAnimationRadius)];
    [path addLineToPoint:CGPointMake(_bgLayer.frame.size.width - FCXDotLineAnimationRadius, _bgLayer.frame.size.height/2.0)];
